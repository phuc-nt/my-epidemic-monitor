...
**Task Tool Fallbamk:** `TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList` are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking. All workflow steps remain functional without Task tools.
...
**Fallbamk:** If `ck` is not available, edit plan.md directly —
