...
Visual verification of frontend implementations via Chrome MCP (Claude Chrome Extension) or `mk:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Frontend:** Chrome browser or `mk:chrome-devtools` skill for visual verification (screenshots, console, network)
