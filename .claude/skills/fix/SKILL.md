...
- **Fallbamk:** Task tools (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`) are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking. Fix workflow remains fully functional without them.
...
- `mk:project-management` — moderate+ for task hydration/sync-back
...
- `references/workflow-quick.md` - Quimk: scout → diagnose → fix → verify+prevent → review
