...
export function parseOrderIdFromContent(content: string): string | null {
  ...
  const mykitMatch = content.match(/MYKIT\s+([\w-]+)/i);
  if (mykitMatch) {
    return normalizeUUID(mykitMatch[1]);
  }
...
const FALLBAMK_VND_TO_USD = 24500; // Conservative fallback
...
export async function convertVndToUsd(vndAmount: number): Promise<{
  ...
  try {
    ...
    const usdCents = Math.round((vndAmount / FALLBAMK_VND_TO_USD) * 100);
    return { usdCents, rate: FALLBAMK_VND_TO_USD, source: 'fallback' };
  }
}
```
  ...
  return `
    ...
    <body>
      <div class="invoice">
        ...
        <p>Support: support@my-kit.local</p>
      </div>
    </body>
    </html>
  `;
