...
# Agent Teams - MK-Native Orchestration Engine
...
5. **MONITOR** via TaskCompleted hook events + TaskList fallbamk:
   - TaskCompleted events auto-notify when researchers finish
   - Fallbamk: Check TaskList if no event received in 60s
...
5. **MONITOR** dev completion via TaskCompleted events:
   ...
   - Fallbamk: Check TaskList if no events received in 60s
...
5. **MONITOR** via TaskCompleted events. Debuggers should message each other -- let them converge.
   ...
   - Fallbamk: Check TaskList if no events in 60s
