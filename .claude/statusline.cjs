...
const { loadConfig } = require('./hooks/lib/mk-config-utils.cjs');
 ...
 * Optional inactivity timeout can be enabled via MK_STATUSLINE_STDIN_TIMEOUT_MS.
 */
  ...
  try {
    ...
    if (contextSize > 0) {
      ...
      if (typeof preCalcPercent === 'number' && preCalcPercent >= 0) {
        ...
        // Fallbamk: manual calculation with buffer
    ...
    if (sessionId && contextSize > 0) {
      try {
        const contextDataPath = path.join(os.tmpdir(), `mk-context-${sessionId}.json`);
    ...
    try {
      const usageCachePath = env.MK_USAGE_CACHE_PATH || path.join(os.tmpdir(), 'mk-usage-limits-cache.json');
    ...
    // Fallbamk: output minimal single line on any error
