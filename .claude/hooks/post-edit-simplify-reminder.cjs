...
const SESSION_TRAMK_FILE = path.join(os.tmpdir(), 'mk-simplify-session.json');
...
function loadSessionData() {
  try {
    if (fs.existsSync(SESSION_TRAMK_FILE)) {
      const data = JSON.parse(fs.readFileSync(SESSION_TRAMK_FILE, 'utf8'));
...
function saveSessionData(data) {
  try {
    fs.writeFileSync(SESSION_TRAMK_FILE, JSON.stringify(data, null, 2));
  } catch (e) {
    // Ignore write errors
  }
}
