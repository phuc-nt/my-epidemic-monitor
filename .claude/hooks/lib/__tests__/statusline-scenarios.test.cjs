...
const USAGE_CACHE_PATH = path.join(os.tmpdir(), `mk-usage-limits-cache-statusline-test-${process.pid}.json`);
...
function runStatuslineSync({ payload, cwd = TEST_ROOT, env = {}, inputRaw = null, timeoutMs = 20000 }) {
  ...
  const result = spawnSync('node', [STATUSLINE_PATH], {
    ...
    input,
    timeout: timeoutMs,
    env: { ...process.env, MK_USAGE_CACHE_PATH: USAGE_CACHE_PATH, ...env }
  });
  if (result.error) {
    throw result.error;
  }
...
async function main() {
  ...
  await test('WSL-style path output works', async () => {
    const payload = {
      model: { display_name: 'Claude' },
      workspace: { current_dir: '/mnt/c/Users/user/project' },
      context_window: { context_window_size: 200000 }
    };
    const result = runStatuslineSync({ payload });
    assertSuccessfulRun(result, 'WSL path scenario');
    const { stdout } = result;
    assertContains(stdout, '/mnt/c/Users/user/project', 'Should preserve WSL path');
  });
