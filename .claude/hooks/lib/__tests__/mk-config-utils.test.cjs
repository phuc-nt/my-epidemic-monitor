...
test('getGitRoot returns null for non-git directory', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-no-git-' + Date.now());
...
test('getGitBranch returns null or empty in detached HEAD state', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-detached-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
...
test('getGitRoot works in detached HEAD state', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-detached-root-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
    execSync('git config user.email "test@test.com"', { cwd: tempDir });
...
test('getGitRoot returns null for bare repository (no working tree)', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-bare-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
...
test('getGitBranch returns null for bare repository (no HEAD ref)', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-bare-branch-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q --bare', { cwd: tempDir });
...
test('getGitRoot returns innermost repo for nested git repos', () => {
  const outerDir = path.join(os.tmpdir(), 'mk-test-nested-outer-' + Date.now());
...
test('getGitRoot from nested subdir returns correct root', () => {
  const outerDir = path.join(os.tmpdir(), 'mk-test-nested-sub-' + Date.now());
  const innerDir = path.join(outerDir, 'inner');
...
test('getGitRoot resolves through symlink to git repo', () => {
  const realDir = path.join(os.tmpdir(), 'mk-test-real-' + Date.now());
  const linkDir = path.join(os.tmpdir(), 'mk-test-link-' + Date.now());
...
test('getGitRoot with symlinked subdirectory', () => {
  const realDir = path.join(os.tmpdir(), 'mk-test-real-sub-' + Date.now());
  const subDir = path.join(realDir, 'subdir');
  const linkToSub = path.join(os.tmpdir(), 'mk-test-link-sub-' + Date.now());
...
test('getGitRoot works with git worktree', () => {
  const mainDir = path.join(os.tmpdir(), 'mk-test-wt-main-' + Date.now());
  const worktreeDir = path.join(os.tmpdir(), 'mk-test-wt-tree-' + Date.now());
...
test('getGitRoot works with unicode characters in path', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-日本語-émoji-🔥-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
...
test('getGitBranch works with unicode branch name', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-unicode-branch-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
    execSync('git config user.email "test@test.com"', { cwd: tempDir });
...
test('getGitRoot works with special shell characters in path', () => {
  ...
  const tempDir = path.join(os.tmpdir(), "mk-test-special-$var-'quote'-" + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
...
test('getGitRoot works on new repo with no commits', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-empty-repo-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
...
test('getGitBranch on new repo returns default branch', () => {
  const tempDir = path.join(os.tmpdir(), 'mk-test-new-repo-branch-' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  try {
    execSync('git init -q', { cwd: tempDir });
