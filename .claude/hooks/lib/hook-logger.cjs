...
const LOMK_FILE = path.join(LOG_DIR, 'hook-log.lock');
const MAX_LINES = 1000;
const TRUNCATE_TO = 500;
const LOMK_TIMEOUT_MS = 250;
const LOMK_RETRY_MS = 10;
...
function withLogLock(fn) {
  ...
  while (Date.now() - startedAt < LOMK_TIMEOUT_MS) {
    let fd;
    try {
      fd = fs.openSync(LOMK_FILE, 'wx');
      try {
        return fn();
      } finally {
        try { fs.closeSync(fd); } catch (_) {}
        try { fs.unlinkSync(LOMK_FILE); } catch (_) {}
      }
    } catch (error) {
