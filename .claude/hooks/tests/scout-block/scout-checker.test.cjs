...
const DEFAULT_OPTS = {
  ...
  ckignorePath: path.join(FIXTURES_DIR, 'mkignore-default.txt'),
...
describe('checkScoutBlock - custom .mkignore', () => {
  const customOpts = {
    ...DEFAULT_OPTS,
    ckignorePath: path.join(FIXTURES_DIR, 'mkignore-custom.txt')
  };
...
describe('checkScoutBlock - negation .mkignore', () => {
  const negOpts = {
    ...DEFAULT_OPTS,
    ckignorePath: path.join(FIXTURES_DIR, 'mkignore-negation.txt')
  };
...
describe('P0 - absolute and relative paths', () => {
  ...
  it('blocks absolute path to dist', () => {
    const r = checkScoutBlock({ toolName: 'Read', toolInput: { file_path: '/Users/user/project/dist/bundle.js' }, options: DEFAULT_OPTS });
    assert.ok(r.blocked);
  });
