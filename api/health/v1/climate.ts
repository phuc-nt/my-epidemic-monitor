/**
 * Climate Predictive Alerts API — edge function.
 * Fetches 14-day weather forecast from Open-Meteo for 8 Vietnam provinces
 * and computes dengue / HFMD risk scores.
 * Cache TTL: 6 hours.
 */
...
/**
 * Dengue risk: peaks with warm temp (25–35°C), significant rainfall (>5mm),
 * and high humidity (>70%), all of which promote Aedes mosquito breeding.
 */
function dengueScore(tempMax: number, rainfall: number, humidity: number): number {
  const tempFactor = tempMax >= 25 && tempMax <= 35 ? 1.0
    : tempMax > 35 ? 0.7   // still risky but slightly less optimal
    : tempMax >= 20 ? 0.3  // cool — lower transmission
    : 0.1;
...
/** HFMD risk: elevated when temp > 28°C and humidity > 80%. */
...
// ---------------------------------------------------------------------------
// Per-province forecast assembly
// ---------------------------------------------------------------------------
...
function buildForecast(province: Province, data: OpenMeteoDaily): ClimateForecast {
  ...
  // Average over the 14-day window
  ...
  // Peak risk day: day with highest combined dengue score
  let peakScore = -1;
...
export default async function GET(_request: Request): Promise<Response> {
  ...
  const results = await Promise.allSettled(
    PROVINCES.map((p) => fetchWeather(p.lat, p.lng)),
  );
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result?.status === 'fulfilled') {
      try {
        forecasts.push(buildForecast(PROVINCES[i]!, result.value.daily));
      } catch {
        // skip malformed response for this province
      }
    }
  }

  if (forecasts.length === 0) {
    return errorResponse('All province weather fetches failed', 502);
  }
